...
- Only to renew master nodes' certificate if etcd doesn't has certificate. [See this](other-en_US.md) (etcd doesn't use TLS encrypted connection by default if the k8s version is less than `v1.9.x`)
...
**Note:** For clutsers of version `v1.15.x` and `v1.16.x`, there is a bug on `kubeadm alpha certs renew <cert_name>` command. You need to handle this mannually. See [this](other-en_US.md)
